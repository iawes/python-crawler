...
def stock_board_cons_ths(symbol: str = "301558") -> pd.DataFrame:
    ...
    for page in tqdm(range(1, page_num + 1), leave=False):
        ...
        print(url)
        r = requests.get(url, headers=headers)
        temp_html = pd.read_html(r.text)
        temp_df = temp_html[0]

        #pd.io.parsers.ParserBase({'names':temp_df.columns})._maybe_dedup_names(temp_df.columns)
        #big_df.reset_index(inplace=True)
        #temp_df.reset_index(inplace=True)
        #df=pd.read_table(name)
        #print(temp_df)
#        aa = big_df.columns.to_list()
#        bb = set(aa)
#       if len(aa) == len(bb):
#           print('没有重复的元素！')
#       else:
...
if __name__ == "__main__":
    gn = stock_board_concept_name_ths()
    print(gn)
    gn.to_csv('./thsgn/gnbk.csv')

    for row in gn.itertuples():
        #print(getattr(row, '概念名称'), getattr(row, '代码'), getattr(row, '成分股数量'))
        #print(type(row))
        code = getattr(row, '代码')
        name = getattr(row, '概念名称')
        name2 = re.sub('([^\u4e00-\u9fa5\u0030-\u0039\u0041-\u007a])', '', name)

        try:
            cf = stock_board_cons_ths(symbol=code)
            #print(cf)
            cf.to_csv('./thsgn/' + code+'.csv')
            cf.to_csv('./thsgn/' + name2+'.csv')
        except:
            print("get %s failed." %(code))

        time.sleep(1)

#    stock_board_concept_cons_ths_df = stock_board_concept_cons_ths(
#        symbol=getattr(row, '概念名称')
#    )
#    print(stock_board_concept_cons_ths_df)
#    stock_board_concept_cons_ths_df.to_csv('111.csv')
