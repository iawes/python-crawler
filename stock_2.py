...
#        print(type(text), len(text))
        print(text[:100]) # 打印text的前100个字符

        # 定义一个正则表达式，匹配<script>和</script>之间的任意内容
        SCRIPT_RE = re.compile(r'\*<script.*?</script>\*/')
        # 将匹配到的内容替换为空字符串
        text = re.sub(SCRIPT_RE, '', text)
        text = text.strip('\n =(')
...
# 定义一个函数，根据股票数据计算布林带指标，并返回中轨线
def get_boll_mid(data):
    # 使用talib库计算布林带指标，参数为收盘价、时间周期、标准差倍数和移动平均类型
    upper, middle, lower = talib.BBANDS(data["close"], timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)
    # 返回中轨线
    return middle

# 定义一个函数，根据股票数据和中轨线判断是否满足策略条件，并返回买入或卖出信号
def get_signal(data, middle):
    # 创建一个空列表，用于存储信号
    signal = []
    # 遍历股票数据的每一行
    for i in range(len(data)):
        # 如果收盘价突破中轨线，并且之后连续三天运行在中轨线之上，则买入信号为1，否则为0
        # 把data["close"]列转换为浮点数类型的
        data["close"] = data["close"].astype(float)
        if data["close"][i] > middle[i] and data["close"][i-1] < middle[i-1] and data["close"][i-2] > middle[i-2] and data["close"][i-3] > middle[i-3]:
            buy = 1
        else:
            buy = 0
        # 如果收盘价跌破中轨线，则卖出信号为-1，否则为0
        if data["close"][i] < middle[i] and data["close"][i-1] > middle[i-1]:
            sell = -1
        else:
            sell = 0
        # 将买入信号和卖出信号相加，得到总信号，并添加到列表中
        signal.append(buy + sell)
    # 返回信号列表
    return signal
...
def backtest(code, start_date, end_date, capital):
    ...
    # 计算中轨线
    middle = get_boll_mid(data)
    # 获取信号列表
    signal = get_signal(data, middle)
